...
const Style1 = (data, marks, $tags, $style, Config) => {
  ...
  const Color = (Array.isArray(Config.color) ? Config.color : [
    '#e5e6e9',  '#e5e6e9', '#e5e6e9','#e5e6e9','#e5e6e9','#e5e6e9','#e5e6e9','#e5e6e9','#e5e6e9','#e5e6e9','#e5e6e9','#e5e6e9','#e5e6e9'
  ...
  $style.innerHTML = genCSS() + `
    #my-tags { overflow-x: hidden; padding: 7%; text-align: center; width: 100%; height: 100%; position: relative; box-sizing: border-box; z-index: 2;}
    #my-tags a::after { content: "🎐"; }
    #my-tags a:hover::after { animation: move .3s linear infinite; position: relative; }
    #my-tags mark { position: absolute; padding: .3em .4em; display: inline-block; border-radius: em; margin: .2em .3em .5em .5em; opacity: .8; transition: opacity .2s; box-shadow: 0.3px 0.5px 5px #111;border-style: double;}
